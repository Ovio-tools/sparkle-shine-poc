...
Contact 1: Deborah Ashworth — commercial lead, referral (Austin TX)
Contact 2: Calvin Trevino  — residential lead, referral (Austin TX)

Run:
    python3 create_contacts_deborah_calvin.py
"""
...
# ---------------------------------------------------------------------------
# Contact details — verified unique against existing HubSpot contacts
# ---------------------------------------------------------------------------
...
CONTACTS = [
    {
        "firstname":          "Deborah",
        "lastname":           "Ashworth",
        "email":              "d.ashworth@ashworthpartners.com",
        "phone":              "(512) 555-0462",
        "company":            "Ashworth & Partners Consulting",
        "address":            "301 Congress Ave Ste 1150",
        "zip":                "78701",
        "city":               "Austin",
        "state":              "TX",
        "lifecyclestage":     "lead",
    ...
    },
    {
        "firstname":          "Calvin",
        "lastname":           "Trevino",
        "email":              "calvin.trevino@gmail.com",
        "phone":              "(512) 555-0529",
        "address":            "2308 Manor Rd",
        "zip":                "78722",
        "city":               "Austin",
        "state":              "TX",
        "lifecyclestage":     "lead",
...
def _email_already_exists(session, email):
...
def create_contact(session, properties):
    resp = session.post(
...
def _next_lead_canonical_id(db, offset=0):
...
def register_in_db(canonical_id, hubspot_id):
...
def main():
    ...
    # Assign canonical IDs from current DB snapshot
    db = get_connection(_DB_PATH)
    db.row_factory = __import__("sqlite3").Row
    entries = []
    for offset, contact in enumerate(CONTACTS):
        canonical_id = _next_lead_canonical_id(db, offset=offset)
        entries.append({"canonical_id": canonical_id, "contact": contact})
    db.close()
    ...
    for entry in entries:
        register_in_db(entry["canonical_id"], entry["hs_id"])
        print(f"  [OK] {entry['canonical_id']} → hubspot:{entry['hs_id']}")
