"""
One-off script: create a single new HubSpot contact for Sparkle & Shine and
register the canonical ID in cross_tool_mapping.

Contact profile: Priya Nair — residential homeowner in North Austin/Allandale
who submitted a website inquiry in late Mar 2026 asking about biweekly service.

Run:
    python3 create_contact_priya_nair.py
"""
...
CONTACT = {
    "firstname":          "Priya",
    "lastname":           "Nair",
    "email":              "priya.nair@northaustin.example.com",
    "phone":              "(512) 634-7821",
    "company":            "",
    "jobtitle":           "",
    "address":            "4205 Balcones Dr",
    "city":               "Austin",
    "state":              "TX",
    "zip":                "78731",
    # Standard HubSpot lifecycle stage
    "lifecyclestage":     "lead",
    # Custom properties (pre-created in Phase 1 Step 6)
    "client_type":        "residential",
    "service_frequency":  "biweekly",
    "neighborhood":       "North Austin/Allandale",
...
def create_contact(session: requests.Session, properties: dict) -> dict:
    # Strip blank strings so HubSpot doesn't reject optional fields
    payload = {k: v for k, v in properties.items() if v != ""}
    url = f"{_BASE_URL}/crm/v3/objects/contacts"
    resp = session.post(url, json={"properties": payload}, timeout=30)
...
def register_in_db(canonical_id: str, hubspot_id: str) -> None:
    ...
    automation layer knows this contact already exists and won't assign a
    duplicate canonical ID on the next sync run.
    """
    db = get_connection(_DB_PATH)
    db.row_factory = __import__("sqlite3").Row
    try:
        # Collision guard: refuse to map the same HubSpot ID twice.
        existing = db.execute(
            "SELECT canonical_id FROM cross_tool_mapping "
...
def main() -> None:
    print("=" * 60)
    print("  Sparkle & Shine — Create New HubSpot Contact")
    print("=" * 60)
