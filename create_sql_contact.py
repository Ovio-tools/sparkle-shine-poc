"""
One-off script: create a single new HubSpot contact and mark it as a
Sales Qualified Lead (SQL).

Contact profile: Marcus Rivera — commercial facilities manager at an
Austin co-working space that just requested a cleaning proposal during
Sparkle & Shine's Mar 2026 spring pipeline push.

Run:
    python create_sql_contact.py
"""
...
CONTACT = {
    "firstname":          "Marcus",
    "lastname":           "Rivera",
    "email":              "m.rivera@capitalcoworks.com",
    "phone":              "(512) 847-3920",
    "company":            "Capital Co-Works",
...
def main() -> None:
    ...
    # Assign the canonical ID before the API call so we can register it
    # immediately on success — even if a later step fails, the mapping exists.
    db = get_connection(_DB_PATH)
    db.row_factory = __import__("sqlite3").Row
    canonical_id = _next_lead_canonical_id(db)
    db.close()

    print(f"  Assigned canonical ID: {canonical_id}")
    print("\nPushing to HubSpot...")
